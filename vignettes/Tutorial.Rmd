...
Let's suppose that we are interested in the evolution of the unemployment rate over the last few quarters. In this case, we can use the `get_many_data()` function, as shown in the following example:

```{r, message=FALSE,warning=FALSE,results='hide'}
ene_serie <-  get_many_data(dataset = "ene", from = "2022-09-aso", to = "2023-07-jja", col_list = c("ano_trimestre", "mes_central", "cae_especifico", "fact_cal"), memory_warning_limit = 10000)
```

Next, we can calculate the unemployment rate and create a graph for it.
