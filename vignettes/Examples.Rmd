...
category_serie <- ene_serie %>% 
  map_df(., . %>% 
           mutate(fact_cal = as.numeric(gsub(",", ".", fact_cal)),
                  ...
                  ocupational_category = ifelse(categoria_ocupacion==1, "Employers",
                                                ifelse(categoria_ocupacion==2, "Own-account workers",
                                                       ifelse(categoria_ocupacion==3, "Private wage earners",
                                                              ifelse(categoria_ocupacion==4, "Public wage earners",
                                                                     ifelse(categoria_ocupacion %in% 5:6, "Domestic workers", "Unpaid family workers")))))) %>%   
           filter(employed == 1) %>% 
